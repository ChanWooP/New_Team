...
$(function() {
	$("body").on("click", ".btnSendEventLike", function() {	
		...
		var url="<%=cp%>/user/event/insertEventLike";  
...
function listPage(page) {
	var url="<%=cp%>/user/event/listReply"
...
$(function() {
	$(".btnSendReply").click(function() {
		...
		var url="<%=cp%>/user/event/insertReply";
		$.ajax({
			type:"post"
			,url:url
			,data:query
	...
	$("body").on("click", ".deleteReply", function() {
		...
		var url="<%=cp%>/user/event/deleteReply";
...
function listReplyAnswer(eventreplyAnswer) {
	var url="<%=cp%>/user/event/listReplyAnswer";
	$.ajax({
		type:"get"
		,url:url
...
function countReplyAnswer(eventreplyAnswer) {
	var url="<%=cp%>/user/event/countReplyAnswer";
	$.ajax({
		type:"post"
		,url:url
		,data:{eventreplyAnswer:eventreplyAnswer}
					...
					<tr>
						<td colspan="2" align="left" style="padding-left: 5px;">
							...
							<c:if test="${not empty ndto}">
								<a href="<%=cp%>/user/event/article?${query}&eventNum=${ndto.eventNum}">${ndto.eventTitle}</a>
							</c:if>
							</td>
					</tr>
					<tr>
						<td colspan="2" align="left" style="padding-left: 5px;">
							...
							<c:if test="${not empty pdto}">
								<a href="<%=cp%>/user/event/article?${query}&eventNum=${pdto.eventNum}">${pdto.eventTitle}</a>
							</c:if>
							</td>
					</tr>
				</tbody>
